...
diracServices(){
  ...
  services=`cat services | cut -d '.' -f 1 | grep -v ^ConfigurationSystem | grep -v SystemAdministrator | grep -v RAWIntegrity | grep -v RunDBInterface | grep -v Strategy | grep -v SiteMap | grep -v ProductionRequest | grep -v MigrationMonitoring | grep -v Future | sed 's/System / /g' | sed 's/Handler//g' | sed 's/ /\//g'`
