...
diracServices(){
  ...
  services=`cat services | cut -d '.' -f 1 | grep -v ^ConfigurationSystem | grep -v SystemAdministrator | grep -v HttpStorageAccess | grep -v RAWIntegrity | sed 's/System//g' | sed 's/Handler//g' | sed 's/ /\//g'`
