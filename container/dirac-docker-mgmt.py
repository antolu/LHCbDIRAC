...
class LHCbDockerMgmt(object):
  ...
  def __chooseMarathonMaster(self):
    ...
    raise requests.ConnectionError("None of the candidate is answering to ping")    
  ...
  def _getRunningDiracComponents(self):
    ...
    if res.status_code != 200:
      logging.error("Error fetching applications: %s %s"%(res.status_code, res.content))
      return
  ...
  def __putConfiguration(self, config):
    ...
    if res.status_code == 200:
      ...
      logging.error("Failed to put configuration %s %s"%(res.status_code, res.content))
  ...
  def __downloadLHCbDiracDockerFilesLocal(self):
    ...
    shutil.copyfile(os.path.join(baseDir, 'dirac_self_ping.py'), 'dirac_self_ping.py')
    shutil.copyfile(os.path.join(baseDir, 'dockerEntrypoint.sh'), 'dockerEntrypoint.sh')

    with open(os.path.join( baseDir, 'Dockerfile.tpl'), 'r') as tpl:
      with open('Dockerfile', 'w') as dockerfile:
        dockerfile.write(''.join(tpl.readlines()).replace('$$LHCB_DIRAC_VERSION$$', self.version))
  ...
  def __downloadLHCbDiracDockerFiles(self):
    ...
    baseUrl = urlparse.urljoin(self.gitRepositoryURL, os.path.join(self.version, 'container/lhcbdirac/'))
    ...
    for fn in ['dirac_self_ping.py', 'dockerEntrypoint.sh', 'Dockerfile.tpl']:
      ...
      if res.status_code != 200:
        logging.error("Could not get %s:%s %s"%(fileUrl, res.status_code, res.content))
  ...
  def dockerPublish(self):
    ...
    # retag 
    tagged =  self.dockerClient.tag(image = 'lhcbdirac:%s'%self.version, repository = self.dockerRepository, tag = self.version)
    if not tagged:
...
if __name__ == '__main__':
  ...
  defaultMarathonMasters = ['http://lbmesosms01.cern.ch:8080', 'http://lbmesosms02.cern.ch:8080', 'http://lbmesosms03.cern.ch:8080']
