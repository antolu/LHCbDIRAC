  ...
  def execute( self ):
    ...
    for dirLfn in self.dirDict.keys():
      ...
      if not res[ 'OK' ]:
        ...
        #self.FillMetadata( self.metadataForAcc, 'na' )
        # this usually happens when directories are removed from LFC between the StorageUsageDB dump and this call, if the Db is refreshed exactly in this time interval. Not really a problem.
        #######################3 just a try ##############################################3
        continue
      if res['Value']:
        for row in res['Value']:
          dirID, configName, configVersion, conditions, processingPass, eventType, fileType, production = row
        ...
        gLogger.info( "No result returned from getDirMetadata => Query Bookkeeping to retrieve meta-data for %s directory and then cache it" % dirLfn )
      ...
      for se in self.PFNUsage[ dirLfn ]:
        ...
        self.fillAndSendAccountingRecord( dirLfn, se, self.metadataForAcc ) 
    ...
    gLogger.notice( "Total calls to Bookkeeping: %d (getDirectoryMetadata: %d, getEventType: %d)" % (totalCallsToBkk, callsToBkkgetDirectoryMetadata, self.callsToBkkGetEvtType ))
    ...
    seList.sort()
    for se in seList:
      gLogger.info("all: %s  %d %d Bytes ( %.2f TB ) " %( se, self.debug_seUsage[ se ]['Files'], self.debug_seUsage[ se ]['Size'], self.debug_seUsage[ se ]['Size']/1.0e12 ))
      if se in self.debug_seUsage_acc.keys():
        gLogger.info("acc: %s  %d %d Bytes ( %.2f TB ) " %( se, self.debug_seUsage_acc[ se ]['Files'], self.debug_seUsage_acc[ se ]['Size'], self.debug_seUsage_acc[ se ]['Size']/1.0e12 ))
      else:
...
#....................................................................................................
  def FillMetadata( self,  dictToFill, metadataValue ):
    ...
    keyList = ['ConfigName','ConfigVersion','FileType','Production','ProcessingPass','ConditionDescription','EventType']
...
#....................................................................................................
  ...
  def generateStorageUsageDictionaryPerDirectory( self ):
    ...
    # raw data:               /lhcb/data/2011/RAW/FULL/LHCb/COLLISION11/99983 => len 9 (under /lhcb/data/ from 2011 only RAW, before 2011 also other file types)
    ...
    gLogger.info("Directories that will be ignored, as their content is not stored in the Bookkeeping: %s " % directoriesNotInBookkeeping )
    for dir in self.totalDirList:
      # make sure that last character is a '/'
      if dir[-1] != "/":
        dir = "%s/" % dir
      splitDir = dir.split('/')
      if len( splitDir ) < 4: # this is just to avoid picking up intermediate directories which don't contain files, like /lhcb/
        gLogger.warn("Directory %s does not contain files" % dir )
        continue
      secDir = splitDir[ 2 ] 
      if secDir in directoriesNotInBookkeeping.keys():
        gLogger.verbose("Not a production directory. Skip it. %s " % dir )
    ...
    gLogger.info("Directories retrieved from StorageUsageDB containing production files: %d " % len( self.dirDict.keys() ) )
    gLogger.info("For the directories containing production files, get the number of files and size from StorageUsage service")
...
#....................................................................................................
  def fillAndSendAccountingRecord( self, lfnDir, se, metadataDict ):
    ...
    if eventType not in self.eventTypeDescription.keys():
      gLogger.notice("Event type description not available for eventTypeID %s, Get the event types from Bkk" % eventType )
      ...
      if not res['OK']:
        ...
        self.eventTypeDescription[ str(eventTypeID) ] = 'na'
      else:
        val = res['Value']
    ...
    gLogger.notice( ">>>>>>>>Send DataStorage record to accounting for fields: DataType: %s Activity: %s FileType: %s Production: %s ProcessingPass: %s Conditions: %s EventType: %s StorageElement: %s --> physFiles: %d  physSize: %d lfnFiles: %d lfnSize: %d " % ( configName, configVersion, fileType, production, processingPass, conditions, eventTypeDescription, se, physicalFiles, physicalSize, logicalFiles, logicalSize ) )

    res = gDataStoreClient.addRegister( dataRecord )
    if not res[ 'OK']:
      self.log.notice( "ERROR: addRegister returned: %s" % res )
      return -1
    self.totalRecords += 1
