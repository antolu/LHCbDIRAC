...
class PluginUtilities:
  ...
  def getBookkeepingMetadata( self, lfns ):
    start = time.time()
    res = self.bkClient.getFileMetadata( lfns )
    self.logVerbose( "Obtained BK metadata of %d files in %.3f seconds" % ( len( lfns ), time.time() - start ) )
    return res
  ...
  def getRAWAncestorsForRun( self, runID, param='', paramValue='' ):
    ...
    if cachedLFNs:
      self.logVerbose( "Cache hit for run %d: %d files cached" % ( runID, len( cachedLFNs ) ) )
      for lfn in cachedLFNs:
        ancestors += cachedLFNs[lfn]
      lfns = [lfn for lfn in lfns if lfn not in cachedLFNs]
    if param:
      res = self.getBookkeepingMetadata( lfns )
      if not res['OK']:
  ...
  def groupByRunAndParam( self, lfns, files, param='' ):
    ...
    for lfn, metadata in res['Value'].items():
      runNumber = metadata.get( "RunNumber" ) if metadata.get( "RunNumber" ) else 0
      runDict.setdefault( runNumber, {} ).setdefault( metadata.get( param ), [] ).append( lfn )
    return S_OK( runDict )

  def groupBySize( self, files, status ):
    requestedSize = float( self.getPluginParam( 'GroupSize', 1 ) ) * 1000 * 1000 * 1000 # input size in GB converted to bytes
    maxFiles = self.getPluginParam( 'MaxFiles', 100 )
    # Get the file sizes
  ...
  def getCachedRunLFNs( self, runID, paramValue ):
    return self.cachedRunLfns.setdefault( runID, {} ).setdefault( paramValue, [] )

  def setCachedRunLfns( self, runID, paramValue, lfnList ):
    self.cachedRunLfns[runID][paramValue] = lfnList
