  ...
  sextrapackages, svisible, sprocessingpass, susable, sdqtag, soptsf, smulti, ssysconfig,\
    ...
    if retVal['OK']:
      parameters = ['StepId', 'StepName', 'ApplicationName', 'ApplicationVersion', 'OptionFiles', 'DDDB',
                    ...
                    'isMulticore', 'SystemConfig', 'RuntimeProjects']
      rParameters = ['StepId', 'StepName', 'ApplicationName', 'ApplicationVersion', 'OptionFiles',
                     ...
                     'OptionsFormat', 'isMulticore', 'SystemConfig']
      records = []
      for record in retVal['Value']:
        step = list( record[0:15] )
        runtimeProject = []
        runtimeProject = [ rec for rec in list( record[15:] ) if rec != None]
...
and files.qualityid= dataquality.qualityid'
  ...
  def getFiles( self, simdesc, datataking, procPass, ftype, evt,
    ...
    statistics = '/*+ INDEX(f FILES_JOB_EVENT_FILETYPE) */ '
    ...
    if nbofEvents:
      command = " select %s sum(f.eventstat) from %s where f.jobid= j.jobid %s " % ( statistics, tables, condition )
    elif filesize:
      command = " select %s sum(f.filesize) from %s where f.jobid= j.jobid %s " % ( statistics, tables, condition )
    else:
      command = " select %s distinct f.filename from %s where f.jobid= j.jobid %s " % ( statistics, tables, condition )
  ...
  def getVisibleFilesWithMetadata( self, simdesc, datataking,
    ...
    command = " select /*+ INDEX(f FILES_JOB_EVENT_FILETYPE) */ distinct f.filename, f.eventstat, j.eventinputstat, j.runnumber, \
  ...
  def getFilesSummary( self, configName, configVersion,
    ...
    command = "select /*+INDEX(f FILES_JOB_EVENT_FILETYPE) PARALLEL(bview)*/ count(*), SUM(f.EventStat), SUM(f.FILESIZE), \
