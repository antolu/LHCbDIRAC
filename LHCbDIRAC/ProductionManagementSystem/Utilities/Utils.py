...
def informPeople( rec, oldstate, state, author, inform ):
  ...
  if authorMail:
    if not state in ['BK Check', 'Submitted']:
      if state == 'BK OK':
        ...
        subj = "DIRAC: the state of Production Request %s is changed to '%s'; %s;%s" % ( reqId, state, rec.get( 'WG', '' ), rec.get( 'RequestName', '' ) )
        body = '\n'.join(
  ...
  if inform:
    subj = "DIRAC: the state of %s Production Request %s is changed to '%s'; %s;%s" % ( rec['RequestType'], reqId, state, rec.get( 'WG', '' ), rec.get( 'RequestName', '' ) )
    body = '\n'.join(
  ...
  if state == 'Accepted':
    subj = "DIRAC: the Production Request %s is accepted; %s;%s" % ( reqId, rec.get( 'WG', '' ), rec.get( 'RequestName', '' ) )
    body = '\n'.join( ["The Production Request is signed and ready to process",
                      "You are informed as member of %s group"] )
    groups = [ 'lhcb_prmgr' ]
  elif state == 'BK Check':
    subj = "DIRAC: new %s Production Request %s; %s;%s" % ( rec.get( 'RequestType', '' ), reqId, rec.get( 'WG', '' ), rec.get( 'RequestName', '' ) )
    ...
    subj = "DIRAC: returned Production Request %s; %s;%s" % ( reqId, reqId, rec.get( 'WG', '' ), rec.get( 'RequestName', '' ) )
