...
# _insert use it, so I can't...
  ...
  def createProductionRequest( self, requestDict, creds ):
    ...
    req = "INSERT INTO ProductionRequests ( " + ','.join( self.requestFields[1:-10] )
  ...
  def getProductionRequest( self, requestIDList, subrequestsFor = 0,
    ...
    try: # test filters
      sfilter = []
      for x in filterIn:
        if not x in self.requestFields[:-10]:
    ...
    if sortBy:
      if not sortBy in self.requestFields[:-10]:
        return S_ERROR( "sortBy field does not exist" )
      if sortOrder != 'ASC':
        sortOrder = 'DESC'

    fields = ','.join( ['t.' + x for x in self.requestFields[:-10]] )
...
#        return S_ERROR("Registered processing pass is required to sign for Tech OK")
  ...
  def updateProductionRequest( self, requestID, requestDict, creds ):
    ...
    '''
    fdict = dict.fromkeys( self.requestFields[4:-10], None )
    rec = {}
    for x in requestDict:
      if x in fdict:
        if requestDict[x]:
    ...
    old = dict( zip( self.requestFields[:-10], result['Value'][0] ) )
  ...
  def __getRequest( self, requestID, connection ):
    ...
    if not result['Value']:
      self.lock.release()
      return S_ERROR( 'The request is no longer exist' )
    rec = dict( zip( self.requestFields[:-10], result['Value'][0] ) )
    return S_OK( rec )
  ...
  def getTrackedInput( self ):
    ...
    rec = []
    for x in result['Value']:
      res = dict( zip( self.requestFields[:-10], x ) )
