...
################ WORKFLOW SECTION ####################################
  ...
  def publishWorkflow(self, wf_name, wf_parent, wf_descr, wf_descr_long, wf_body, authorDN, authorGroup, update=False):
    ...
    if not self._isWorkflowExists(wf_name):
      ...
      result = self._insert( 'Workflows', [ 'WFName', 'WFParent', 'Description', 'LongDescription',
                                            'AuthorDN', 'AuthorGroup', 'Body' ],
      ...
      if result['OK']:
        ...
        _msgTuple = (wf_name, authorDN, result['Message'])
        error = 'Workflow "%s" FAILED to be published by DN="%s" with message "%s"' % _msgTuple 
        gLogger.error(error)
      ...
      if update: # we were asked to update
        ...
        if not result['OK']: 
          return result
        # we have to remove trailing " left by self._escapeString()
        wf_body_esc = result['Value'][1:len(result['Value'])-1] 
        result = self._escapeString( wf_descr_long )
        if not result['OK']: 
          return result
        # we have to remove trailing " left by self._escapeString()
        wf_descr_long_esc = result['Value'][1:len(result['Value'])-1] 
  ...
  def getWorkflowInfo(self, name):
    ...
    if result['OK']:
      ...
      newres = {
                 ...
                 'AuthorDN'        : pr[4], 
                 'AuthorGroup'     : pr[5], 
                 'PublishingTime'  : pr[6].isoformat(' ')
                 }
      return S_OK(newres)
    else:
      return S_ERROR('Failed to retrive Workflow with the name=%s \n %s' % (name, result['Message']) )
