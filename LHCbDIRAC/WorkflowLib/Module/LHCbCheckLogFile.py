########################################################################
# $Id: LHCbCheckLogFile.py,v 1.19 2008/05/08 06:11:24 rgracian Exp $
########################################################################
""" Base LHCb Gaudi applications log checking utility """

__RCSID__ = "$Id: LHCbCheckLogFile.py,v 1.19 2008/05/08 06:11:24 rgracian Exp $"
...
#      self.module.step.job.addParameter('Number_OF_Files_non_processed',PARAMETER,str(poolroot))
      ...
      if nprocessed == EvtMax or nomore == 1:
        ...
        if EvtMax != -1 and nprocessed != EvtMax:
          ...
          self.log.info('loop errors %s ' % (str(n)))
          if n == 0:
            file_end = True
          if file_end == False:
