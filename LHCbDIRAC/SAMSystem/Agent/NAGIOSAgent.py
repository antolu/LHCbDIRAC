...
class NAGIOSAgent( AgentModule ):
  ...
  def initialize( self ):
    ...
    gMonitor.registerActivity( "TotalSites", "Total Sites", "NAGIOSAgent", "Sites", gMonitor.OP_SUM, 3600 * 2 )
    gMonitor.registerActivity( "ActiveSites", "Active Sites", "NAGIOSAgent", "Sites", gMonitor.OP_SUM, 3600 * 2 )
    gMonitor.registerActivity( "BannedSites", "Banned Sites", "NAGIOSAgent", "Sites", gMonitor.OP_SUM, 3600 * 2 )
    gMonitor.registerActivity( "DeletedJobs", "Deleted Jobs", "NAGIOSAgent", "Jobs", gMonitor.OP_SUM, 3600 * 2 )

    self.samPub = SAMPublisher()

    result = self.samPub.install()
    if not result['OK']:
      gLogger.error( "NAGIOS publisher installation", result['Message'] )
