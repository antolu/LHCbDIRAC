...
class SAMFinalization( ModuleBaseSAM ):
  ...
  def execute( self ):
    ...
    if not 'SAMResults' in self.workflow_commons:
      ...
      return self.finalize( 'No SAM results found', 'Status ERROR (= 2)', 'error' )
    ...
    #FIXME: what is SAMResults ??
    if 'CE-lhcb-lock' in samResults:
      if not int( samResults[ 'CE-lhcb-lock' ] ) > int( self.nagiosStatus[ 'ok' ] ):
...
#      return self.finalize( 'Failure while publishing SAM results', result['Message'], 'error' )
  ...
  def __getSAMStatus( self, testStatus ):
    ...
    statusString = ''
    for status, localid in self.nagiosStatus.iteritems():
  ...
  def __checkTestsStatus( self, samResults, samLogs ):
    ...
    for testName, testStatus in samResults.iteritems():
      if testName in samLogs:
        if int( testStatus ) > int( self.nagiosStatus[ 'ok' ] ):
  ...
  def __getSoftwareReport( self, samResults, samLogs ):  
    ...
    if int( samResults[ 'CE-lhcb-install' ] ) > int( self.nagiosStatus[ 'ok' ] ):
      self.log.info( 'Software install test was not run by this job.' )
...
#       directory if the publishing flag is set to True.
#    '''
#    
