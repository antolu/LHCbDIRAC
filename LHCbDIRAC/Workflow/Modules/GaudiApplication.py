  ...
  def _findOutputs( self, stepOutput ):
    ...
    for output in stepOutput:
      ...
      if outType in ['dst', 'mdst'] or re.match( '[a-z,A-Z,.]*dst', outType ):
        if output['outputDataName'] in os.listdir( '.' ):

          self.log.info( 'Found output file matching %s' % output['outputDataName'] )
          strippingFiles.append( output )

      else:
        nonStrippingFiles.append( output )
