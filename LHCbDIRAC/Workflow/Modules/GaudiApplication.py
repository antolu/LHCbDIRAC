...
#    result = {'OK':True,'Value':(0,'Disabled Execution','')}
    ...
    #I want to have only lower case files
    ...
    #ADDED MC Simulation to handle the stripping case for the 2011 MonteCarlo workflow
    streamingJobTypes = ['datastripping', 'datareconstruction', 'hospital', 'mcsimulation']
    if self.jobType.lower() in streamingJobTypes:

#      if  self.applicationName.lower() == 'davinci' \
#          and ( re.match( '[a-z,A-Z]*.dst', self.applicationType.lower() ) or self.applicationType.lower() in ['dst', 'mdst'] ) \
#          and self.inputDataType.lower() in ['sdst', 'dst']:
...
#            if output['outputDataType'].lower() in ['dst', 'mdst']:
            stepOutput = output['outputDataName']

            self.log.verbose( 'Checking for presence of "%s" after execution...' % ( stepOutput ) )
            if os.path.exists( stepOutput ):
              if self.jobType.lower() in ['datastripping', 'datareconstruction']:
                self.log.info( 'Assume this is a non-streaming DaVinci step since %s was produced' % ( stepOutput ) )
              elif self.jobType.lower() in ['mcsimulation']:
                self.log.info( 'Assume this is a non-stripping DaVinci step of MC since %s was produced' % ( stepOutput ) )

        else:
          result = self.manageDaVinciStreamingStep()
          if not result['OK']:
            return result
  ...
  def manageDaVinciStreamingStep( self ):
    ...
    if self.jobType.lower() in ['datastripping', 'datareconstruction']:
      self.log.info( 'DaVinci Streaming DST step, will attempt to add output data files to the global list of output data' )
    if self.jobType.lower() in ['mcsimulation']:
      self.log.info( 'MC stripping DST step, will attempt to add output data files to the global list of output data' )
    finalOutputs = []
    toMatch = ''
    outputDataSE = ''
    bkFileTypes = []
