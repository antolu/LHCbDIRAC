...
class NagiosProbesPolicy_TestCase( unittest.TestCase ):
  ...
  def setUp( self ):
    ...
    import LHCbDIRAC.ResourceStatusSystem.Policy.NagiosProbes_Policy as moduleTested
...
class NagiosProbesPolicy_Success( NagiosProbesPolicy_TestCase ):
  ...
  def test_instantiate( self ):
    ...
    '''  
    p = self.policy()
    self.assertEqual( 'NagiosProbes_Policy', p.__class__.__name__ )
  ...
  def test_evaluate_ok( self ):
    ...
    res = p.evaluate()
    self.assertEqual( res[ 'Status' ], 'Error' )    
    p.commandRes = { 'OK' : True, 'Value' : { 'CRITICAL' : 1 } }
    res = p.evaluate()
