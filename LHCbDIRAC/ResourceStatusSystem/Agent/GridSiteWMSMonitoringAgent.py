...
class GridSiteWMSMonitoringAgent(AgentModule):
  ...
  def execute(self):
    ...
    fileContents = ''
    hrefTemp = self.hrefPrefix + \
        '#ds9:_plotNames12:NumberOfJobss13:_timeSelectors5:86400s7:_' + \
        'Statuss7:Runnings5:_Sites%d:%ss9:_typeNames10:WMSHistorys9:_groupings4:Sitee'
  ...
  def _commitFileContents(self, fileData):
    ...
    os.close(fd)
    dm = DataManager()
    result = dm.put("/lhcb/monitoring/lhcb.siteWMSmonitoring.csv", fName,
                    self.opHelper.getValue('LogStorage/LogSE', 'LogSE'))
    try:
      os.unlink(fName)
    ...
    for siteTuple in result['Value']:
      ...
      if site:
        ...
        if not interval:
          ...
          if result['OK']:
            if 'CPUTime' not in monDict[site][jobType]:
              monDict[site][jobType]['CPUTime'] = result['Value']['CPUTime']
            else:
              monDict[site][jobType]['CPUTime'] += result['Value']['CPUTime']
            if 'WallClockTime' not in monDict[site][jobType]:
