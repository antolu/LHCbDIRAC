...
class ShiftDBAgent( AgentModule ):
  ...
  def initialize( self ):
    ...
    self.wsdl         = 'https://preprodcra-ws.cern.ch/cra-ws/CraEgroupsWebService.wsdl'
...
#        return S_OK()
  ...
  def __addMember( self, email, client, wgroup ):
    ...
    if not wgroup.result.Members:
      wgroup.Members = client.factory.create( 'ns1:MembersType' )  

    wgroup.result.Members[ 0 ].append( newmember )
