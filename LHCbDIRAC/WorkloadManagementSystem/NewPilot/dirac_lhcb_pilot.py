...
class CliParams( object ):
  ...
  def __init__( self ):
    ...
    # FIXME: do we actually need all this ??
...
def parseCLIParams():
  ...
  optList, _args = getopt.getopt( sys.argv[1:],
  ...
  for o, v in optList:
    if o in ( '-h', '--help' ):
      ...
      install_opts.append( '-b' )
    elif o == '-d' or o == '--debug':
      cli_params.debug = True
      install_opts.append( '-d' )
    elif o == '-e' or o == '--extraPackages':
      install_opts.append( '-e "%s"' % v )
    elif o == '-g' or o == '--grid':
      cli_params.gridVersion = v
      if v:
        installOpts.append( "-g '%s'" % v )      
    elif o == '-i' or o == '--python':
      cli_params.pythonVersion = v
      if v:
        installOpts.append( '-i "%s"' % v )
    elif o in ( '-l', '--project' ):
      install_opts.append( "-l '%s'" % v )
      cli_params.releaseProject = v
    elif o == '-n' or o == '--name':
      configure_opts.append( '-n "%s"' % v )
      cli_params.site = v
    elif o == '-p' or o == '--platform':
      install_opts.append( '-p "%s"' % v )
      cli_params.platform = v
    elif o == '-r' or o == '--release':
      install_opts.append( '-r "%s"' % v )
      cli_params.releaseVersion = v
    elif o == '-t' or o == '--test':
      cli_params.dryRun = True
    elif o == '-u' or o == '--url':
      install_opts.append( '-u "%s"' % v )
    elif o == '-N' or o == '--Name':
      configure_opts.append( '-N "%s"' % v )
      cli_params.ceName = v
    elif o == '-D' or o == '--disk':
      try:
        cli_params.minDiskSpace = int( v )
      except ValueError:
        pass
    elif o == '-M' or o == '--MaxCycles':
      try:
        cli_params.maxCycles = min( cli_params.maxCycles, int( v ) )
      except ValueError:
        pass
    elif o == '-R' or o == '--reference':
      cli_params.pilotReference = v
    elif o in ( '-S', '--setup' ):
      configure_opts.append( '-S "%s"' % v )
    elif o in ( '-C', '--configurationServer' ):
      configure_opts.append( '-C "%s"' % v )
    elif o in ( '-P', '--path' ):
      install_opts.append( '-P "%s"' % v )
      cli_params.rootPath = v
    elif o in ( '-T', '--CPUTime' ):
      cli_params.jobCPUReq = v
    elif o in ( '-G', '--Group' ):
      cli_params.userGroup = v
    elif o in ( '-O', '--OwnerDN' ):
      cli_params.userDN = v
    elif o in ( '-U', '--Upload' ):
      #TODO
      ...
      install_opts.append( '-V "%s"' % v )
      configure_opts.append( 'defaults-%s.cfg' % v )
    elif o in ( '-W', '--gateway' ):
      configure_opts.append( '-W "%s"' % v )
    elif o == '-E' or o == '--server':
      install_opts.append( '-t "server"' )
    elif o == '-o' or o == '--option':
      configure_opts.append( '-o "%s"' % v )
    elif o == '-s' or o == '--section':
      configure_opts.append( '-s "%s"' % v )
    elif o == '-c' or o == '--cert':
      configure_opts.append( '--UseServerCertificate' )

  return cli_params, configureOpts, install_opts

cliParams, configureOpts, installOpts = parseCLIParams()

#...............................................................................

#FIXME: python loads at startup PYTHONPATH and pushes it to sys.path.. 
#FIXME: if paths are duplicated, who cares ? Does the order matter ??
def checkPATH():
  ...
  for ppath in pythonpath:
    ...
    #FIXME: do we have to use normpath ??
    try:
      if os.path.normpath( ppath ) in sys.path:
        # In case a given directory is twice in PYTHONPATH it has to removed only once
        sys.path.remove( os.path.normpath( ppath ) )
        
    except Exception, x:
      print 'Directories in PYTHONPATH:', pythonpath
      print 'Failing path:', ppath, os.path.normpath( ppath )
...
def __log( level, msg ):
  for _line in msg.split( '\n' ):
    print "%s UTC dirac-pilot [%s] %s" % ( time.strftime( '%Y-%m-%d %H:%M:%S', time.gmtime() ), level, _line )
  sys.stdout.flush()
...
def executeAndGetOutput( cmd ):
  ...
  Executes cmd on a new process and returns exitCode and standard output.  
  """

  child = subprocess.Popen( cmd, shell = True, stdout = subprocess.PIPE,
                            stderr = subprocess.PIPE, close_fds = True )
  
  outData    = child.stdout.read().strip()
...
def getInstallScript( original_root_path, root_path ):
  ...
  for rpath in ( pilotRootPath, original_root_path, root_path ):
    install_script = os.path.join( rpath, installScriptName )
    if os.path.isfile( install_script ):
      break
  else:
    logERROR( "%s requires %s to exist in one of: %s, %s, %s" % ( pilotScriptName, installScriptName,
                                                                  pilotRootPath, original_root_path, 
                                                                  root_path ) )
    logINFO( "Trying to download it to %s..." % original_root_path )
    ...
    try:
      ...
      install_script = os.path.join( original_root_path, installScriptName )
      localFD = open( install_script, "w" )
...
#FIXME: this could be done on a better way
if cliParams.rootPath is not None:
  rootPath = cliParams.rootPath

installScript = getInstallScript( originalRootPath, rootPath )
