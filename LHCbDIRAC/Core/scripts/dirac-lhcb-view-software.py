...
"""
  dirac-lhcb-view-software
    ...
    Usage:
      dirac-lhcb-view-software
        --systemConfig        Optional System Configuration To Query For (by default it does not look for a specific configuration)
...
def printSoftware( package, packageArch ):
  adj = 30
  gLogger.notice( package.split( '.' )[0].ljust( adj ) + package.split( '.' )[1].ljust( adj ) + ','.join( packageArch ).ljust( adj ) )
...
"""List the installed packages. If the 'configuration' argument is set, only this system configuration is listed"""
