  ...
  if result['OK']:
    try:
      for status in result['Value']:
        if status[0] == 'Running' and ('error' in status[2].lower()
                                       or 'not found' in status[2].lower()
                                       or 'exited with status' in status[2].lower()):
          failed_time = status[3]
          return failed_time, status[2]
        elif status[0] == 'Completed' and ('error' in status[1].lower()
                                           or 'not found' in status[1].lower()
                                           or 'exited with status' in status[1].lower()):
          failed_time = status[3]
