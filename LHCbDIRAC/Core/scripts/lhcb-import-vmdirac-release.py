...
if __name__ == "__main__":
  ...
  svnRelease = "%s/tags/%s/%s" % ( project, module, release )

  exitStatus, data = packageDistribution.doLS( '%s/tags/%s' % ( project, module ) )
  if exitStatus:
    createdReleases = []
  else:
  ...
  for diracSystem in createdSystem:
    exitStatus, data = packageDistribution.doLS( '%s/tags/%s/%s' % ( project, module, diracSystem ) )
    if exitStatus:
      createdReleases = []
    else:
      createdReleases = [ v.strip( "/" ) for v in data.split( "\n" ) if v.find( "/" ) > -1 ]
    ...
    if release in createdReleases:
      ...
      systemRelease = os.path.join( project, 'tags', module, diracSystem, release )
      trunkSystem = os.path.join( project, 'trunk', module, diracSystem, 'cmt' )
